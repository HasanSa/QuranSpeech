...
extension QSAyah  {
  init?(json: JSONDictionary) {
    guard let chapter = json["chapter_id"] as? Int,
      let verse = json["verse_number"] as? Int else { return nil }
    
    guard QSQuran.SuraNumberOfAyahs.count != chapter  && QSQuran.SuraNumberOfAyahs[chapter] != verse else {
      return nil
    }
    
    self.sura = chapter
    self.ayah = verse
    self.text = json["text_madani"] as? String
  }
}
